...
/**
 * Pick up to `count` games to recommend when a search finds nothing.
 *
 * The rotation is deterministic per query (a simple string hash picks the
 * starting index), so the same failed search always suggests the same games
 * while different queries surface different corners of the archive.
 */
export function pickRecommendations(games, query, count = 3) {
  ...
  const start = hash % games.length;

  const recommendations = [];
  for (let i = 0; i < games.length && recommendations.length < count; i++) {
    recommendations.push(games[(start + i) % games.length]);
  }
